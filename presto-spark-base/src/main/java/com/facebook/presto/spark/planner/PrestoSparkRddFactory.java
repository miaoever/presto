...
public class PrestoSparkRddFactory
{
            ...
            Class<T> outputType)
    {
        ...
        List<PrestoSparkTaskSource> tableScans = findTableScanNodes(fragment.getRoot());
    ...
    private PrestoSparkTaskSourceRdd createTaskSourcesRdd(
            ...
            List<PrestoSparkTaskSource> sources,
            ...
            Optional<Integer> numberOfShufflePartitions)
    {
        ...
        for (PrestoSparkTaskSource source : sources) {
    ...
    private static List<PrestoSparkTaskSource> findTableScanNodes(PlanNode node)
    {
    ...
    private static List<PrestoSparkTaskSource> findTableScanNodesInternal(NativeExecutionNode node)
    {
        return searchFrom(node.getSubPlan())
                .where(TableScanNode.class::isInstance)
                .findAll().stream().map(t -> new PrestoSparkTaskSource(node.getId(), t)).collect(Collectors.toList());
    }

    private static List<PrestoSparkTaskSource> findTableScanNodesInternal(PlanNode node)
    {
        return searchFrom(node)
                .where(TableScanNode.class::isInstance)
                .findAll().stream().map(t -> new PrestoSparkTaskSource(t.getId(), t)).collect(Collectors.toList());
    }
    ...
    private static class PrestoSparkTaskSource
    {
        private final PlanNodeId sourceId;
        private final PlanNode sourceNode;

        public PrestoSparkTaskSource(PlanNodeId sourceId, PlanNode sourceNode)
        {
