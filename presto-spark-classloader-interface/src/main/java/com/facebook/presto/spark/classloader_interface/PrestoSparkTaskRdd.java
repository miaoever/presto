        ...
        extends ZippedPartitionsBaseRDD<Tuple2<MutablePartitionId, T>>
{
    ...
    public Iterator<Tuple2<MutablePartitionId, T>> compute(Partition split, TaskContext context)
    {
        ...
        List<PrestoSparkShuffleTaskSource> shuffleTaskSources = new ArrayList<>();
        Map<Integer, PrestoSparkShuffleDescriptor> shuffle = new HashMap<>();
        if (isNativeExecutionEnabled) {
            ...
            shuffleHandle.ifPresent(handle -> shuffleDescriptorMap.put(split.index(), new PrestoSparkShuffleDescriptor(split.index(), Optional.empty(), handle)));
        }
        ...
        for (int inputIndex = 0; inputIndex < shuffleInputRdds.size(); inputIndex++) {
            ...
            shuffleInputIterators.put(
                    ...
                    shuffleInputRdds.get(inputIndex).iterator(partition, context));
            if (isNativeExecutionEnabled) {
                checkArgument(partition instanceof ShuffledRDDPartition, "partition is required to be ShuffledRddPartition");
                // shuffleDescriptorMap.put(split.index(), new PrestoSparkShuffleDescriptor(split.index(), Optional.of(partition), getShuffleHandle(shuffleInputRdds.get(inputIndex))));
                shuffleTaskSources.add(new PrestoSparkShuffleTaskSource())
            }
        }
        ...
        // shuffleInputDescritors, shuffleWriteInfo
        return taskProcessor.process(taskSourceIterator, unmodifiableMap(shuffleInputIterators), shuffleDescriptorMap);
    }
